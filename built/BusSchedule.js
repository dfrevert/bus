...
function getCurrentPastChoicesKey(dDay = 0, dHour = 0) {
    if (dDay === undefined || dDay === null) {
        dDay = 0;
    }
    if (dHour === undefined || dHour === null) {
        dHour = 0;
    }
    let now = new Date();
...
function requestVehiclesOnRoute(route) {
    if (_isDebugging) {
...
function populateVehiclesOnRoute(route, responseText) {
...
// eslint-disable-next-line no-unused-vars
function routeDirectionClicked(route, direction_id) {
...
function requestRouteDirectionStops(route, direction_id, shouldCreateButtons) {
...
function populateRouteDirectionStops(route, direction_id, responseText) {
...
function routeDirectionStopClicked(route, direction_id, stop, stopDescription) {
    if (_isDebugging) {
        console.log("routeDirectionStopClicked(route, direction_id, stop, stopDescription)  route=", route, "direction_id=", direction_id, "stop=", stop, "stopDescription=", stopDescription);
    }
    _myRoute = route;
    _myDirection = direction_id;
...
function getVehiclesAndDepartures(route, direction_id, stop) {
    requestVehiclesOnRoute(route);
    requestDepartures(route, direction_id, stop);
    rewriteActualTableData(route, null);
}
// --------------------- Departures
function requestDepartures(route, direction_id, stop) {
    //  Scheduled stops
    if (_isDebugging) {
...
function populateDepartures(route, direction_id, stop, responseText) {
    let busAtPoint;
    let milesAway;
        ...
        if (arr[i].actual === undefined || arr[i].actual === null || !arr[i].actual) {
            ...
            if (actualBlockNumber && actualBlockNumber.length > 1) {
                ...
                */
                ...
                //		let elapsedPhrase = "";
                let minutesSeconds = "";
                if (busAtPoint === null || busAtPoint === void 0 ? void 0 : busAtPoint.locationTime) {
                    ...
                    if (_isDebugging) {
                        ...
                        console.log("populateDepartures busAtPoint=", busAtPoint, " busAtPoint?.locationTime=", busAtPoint === null || busAtPoint === void 0 ? void 0 : busAtPoint.locationTime, " minutesSeconds=", minutesSeconds);
                    }
                }
...
function getStopDescription(route, direction_id, stop) {
    let p = _tblRouteDirectionStops.getByKey(route + '.' + direction_id.toString());
    if (p) {
...
function directionFromDirectionAndRoute(direction_id, route) {
...
function directionAsNumber(obj) {
    if (obj && obj.direction_name) {
        if (obj.direction_name === "Northbound")
            return 0;
        if (obj.direction_name === "Southbound")
            return 1;
        if (obj.direction_name === "Eastbound")
            return 2;
        if (obj.direction_name === "Westbound")
            return 3;
        return -1;
    }
    if (obj && obj.direction_text) {
        if (obj.direction_text === "NB")
            return 0;
        if (obj.direction_text === "SB")
            return 1;
        if (obj.direction_text === "EB")
            return 2;
        if (obj.direction_text === "WB")
            return 3;
        return -1;
    }
}
...
function milesAndDirection(miles) {
    if (miles === undefined || miles === null)
        return;
    if (180.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 180.0 + 11.25) {
        return miles.between.toFixed(2) + " S";
    }
    if (-180.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -180.0 + 11.25) {
        return miles.between.toFixed(2) + " S";
    }
    if (-157.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -157.5 + 11.25) {
        return miles.between.toFixed(2) + " SSW";
    }
    if (-135.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -135.0 + 11.25) {
        return miles.between.toFixed(2) + " SW";
    }
    if (-112.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -112.5 + 11.25) {
        return miles.between.toFixed(2) + " WSW";
    }
    if (-90.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -90.0 + 11.25) {
        return miles.between.toFixed(2) + " W";
    }
    if (-67.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -67.5 + 11.25) {
        return miles.between.toFixed(2) + " WNW";
    }
    if (-45.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -45.0 + 11.25) {
        return miles.between.toFixed(2) + " NW";
    }
    if (-22.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < -22.5 + 11.25) {
        return miles.between.toFixed(2) + " NNW";
    }
    if (0.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 0.0 + 11.25) {
        return miles.between.toFixed(2) + " N";
    }
    if (22.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 22.5 + 11.25) {
        return miles.between.toFixed(2) + " NNE";
    }
    if (45.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 45.0 + 11.25) {
        return miles.between.toFixed(2) + " NE";
    }
    if (67.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 67.5 + 11.25) {
        return miles.between.toFixed(2) + " ENE";
    }
    if (90.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 90.0 + 11.25) {
        return miles.between.toFixed(2) + " E";
    }
    if (112.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 112.5 + 11.25) {
        return miles.between.toFixed(2) + " ESE";
    }
    if (135.0 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 135.0 + 11.25) {
        return miles.between.toFixed(2) + " SE";
    }
    if (157.5 - 11.25 < miles.angleInDegrees && miles.angleInDegrees < 157.5 + 11.25) {
        return miles.between.toFixed(2) + " SSE";
    }
...
function addMarker(busLocation) {
    ...
    for (i = 0; i < _markers.length; i++) {
        ...
        if (markerPosition === undefined || markerPosition === null)
            return;
        if (busLocation.latitude === markerPosition.lat && busLocation.longitude === markerPosition.lng)
            return;
    }
...
var _Washington300N = { latitude: 44.984393, longitude: -93.272505 }; // #14
...
function pastChoicesIndex(pastChoicesArray, findChoice) {
    ...
    return pastChoicesArray.findIndex(x => (findChoice.route !== undefined
...
function hourOfDayAsMinimumString(hourOfDay) {
    if (hourOfDay === 0)
        return '12am';
    if (1 <= hourOfDay && hourOfDay <= 11)
        return hourOfDay.toString() + 'am';
    if (hourOfDay === 12)
        return '12pm';
    if (13 <= hourOfDay && hourOfDay <= 23)
        return (hourOfDay - 12).toString() + 'pm';
    return '??';
}
